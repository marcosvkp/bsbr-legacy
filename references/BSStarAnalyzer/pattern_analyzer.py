...
pattern_analyzer.py
────────────────────────────────────────────────────────────────────────────
Análise profunda de padrões de Beat Saber.

Detecta e quantifica os seguintes padrões:
  - Streams (notas rápidas consecutivas por mão)
  - Jumps (notas simultâneas nas duas mãos)
  - Handclaps / Claps (notas simultâneas com colisão provável)
  - Crossovers (notas cruzando a linha central)
  - Doubles (notas rápidas duplas — "doubles" ou "bursts")
  - Stacks / Towers (notas na mesma posição consecutiva)
  - Sliders / Arcs (V3)
  - Chains / Burst Sliders (V3)
  - Parity breaks (quebra de fluxo de corte)
  - Reset intensity (quanto o jogador precisa "resetar" o pulso)
  - Hand dominance (assimetria de trabalho entre mãos)
  - Obstacle interaction (paredes que forçam movimento corporal)
  - Bomb dodge density (bombas que exigem esquiva)
  - Linear vs Tech ratio (mapas lineares vs maps que exigem ângulos especiais)
"""
...
from parser.enums import NoteCutDirection, NoteColor
...
# Limiar de tempo (em segundos) para considerar notas "simultâneas"
SIMULTANEOUS_THRESHOLD_SEC = 0.020   # 20ms

# Limiar de tempo (em beats) para streams — nota por mão a cada ≤ N beats
STREAM_BEAT_THRESHOLD = 0.27         # ~1/4 beat + tolerância

# Limiar de tempo (em beats) para doubles/bursts
DOUBLE_BEAT_THRESHOLD = 0.13         # ~1/8 beat + tolerância

# Quantidade mínima de notas consecutivas para ser um stream
STREAM_MIN_LENGTH = 5

# Definição de "cruzamento": nota da mão esquerda (c=0) em coluna ≥ 2
#                             nota da mão direita (c=1) em coluna ≤ 1
LEFT_CROSSOVER_COL  = 2   # ≥ esta coluna = crossover para esquerda
RIGHT_CROSSOVER_COL = 1   # ≤ esta coluna = crossover para direita

# Grid Beat Saber: 4 colunas (0-3), 3 linhas (0-2)
GRID_COLS = 4
GRID_ROWS = 3

# Direções "lineares" (sem ângulo especial)
LINEAR_DIRECTIONS = {
    NoteCutDirection.UP,
...
# Mapeamento direção → ângulo (graus)
DIR_TO_ANGLE: Dict[int, float] = {
    0: 180.0,   # UP   → corte de baixo pra cima
    1: 0.0,     # DOWN → corte de cima pra baixo
    2: 270.0,   # LEFT
    3: 90.0,    # RIGHT
    4: 225.0,   # UP_LEFT
    5: 135.0,   # UP_RIGHT
    6: 315.0,   # DOWN_LEFT
    7: 45.0,    # DOWN_RIGHT
    8: -1.0,    # ANY (dot note) — sem ângulo
}

# Parity: qual ângulo é "natural" após o anterior (fluxo ideal)
# Simplificado: direções opostas têm boa paridade de corte
...
def angle_diff(a1: float, a2: float) -> float:
    """Diferença mínima entre dois ângulos em graus."""
...
def detect_streams(hand_notes: List[Note], bpm: float) -> Dict[str, Any]:
    ...
    Retorna métricas: count de streams, comprimento médio, comprimento máximo, ratio de notas em stream.
    """
...
def detect_jumps(all_notes: List[Note], bpm: float) -> Dict[str, Any]:
    """
    Jumps: momentos onde ambas as mãos recebem notas simultaneamente com
    espaço grande entre elas (distância ≥ 2 na grid).
    """
    jump_count = 0
...
def detect_crossovers(hand_notes: List[Note], hand: int) -> Dict[str, Any]:
    """
    Crossovers: mão esquerda (hand=0) em coluna ≥ 2 ou mão direita (hand=1) em coluna ≤ 1.
    Indica dificuldade de movimento corporal e de rastreamento visual.
    """
    crossover_count = 0
...
def detect_doubles(hand_notes: List[Note]) -> Dict[str, Any]:
    """
    Doubles / Bursts: pares de notas extremamente rápidas por mão (≤ DOUBLE_BEAT_THRESHOLD beats).
    Comum em maps tech de alto nível.
    """
    double_count = 0
    for i in range(1, len(hand_notes)):
...
def detect_stacks(all_notes: List[Note], bpm: float) -> Dict[str, Any]:
    """
    Stacks / Towers: múltiplas notas na mesma posição de grid (x, y) quase ao mesmo tempo.
    Difíceis de distinguir visualmente.
    """
    stack_count = 0
    i = 0
    while i < len(all_notes) - 1:
...
def detect_parity_breaks(hand_notes: List[Note]) -> Dict[str, Any]:
    """
    Parity breaks: sequências onde a direção de corte "quebra" o fluxo natural.
    Ex: UP → UP (sem troca de pulso), que força um reset de pulso.
    Mapas com muitos resets são mais difíceis de aprender.
    """
    if len(hand_notes) < 2:
    ...
    total_with_parity = sum(
        1 for n in hand_notes if int(n.d) != 8
    )
...
def detect_linear_vs_tech(all_notes: List[Note]) -> Dict[str, Any]:
    """
    Linear vs Tech:
    - Linear: cortes nas 4 direções principais (up/down/left/right) ou dot
    - Tech: diagonais e ângulo de offset (a ≠ 0)
    Ratio alto = mapa mais técnico.
    """
    linear_count = 0
    tech_count = 0
...
def detect_hand_dominance(left_notes: List[Note], right_notes: List[Note]) -> Dict[str, Any]:
    """
    Dominância de mão: quanto uma mão tem mais notas que a outra.
    Valor 0 = perfeitamente simétrico, 1 = tudo em uma mão.
    """
...
def detect_obstacle_density(obstacles: List[Obstacle], duration_seconds: float) -> Dict[str, Any]:
    """
    Obstáculos por segundo — indica quando o jogador precisa desviar
    ativamente com o corpo (duck walls, side walls).
    """
...
def detect_bomb_density(bombs: List[Note], duration_seconds: float) -> Dict[str, Any]:
    """Bombas por segundo — exigem esquiva ativa."""
...
def detect_vision_blocks_advanced(all_notes: List[Note], bpm: float) -> Dict[str, Any]:
    """
    Vision blocks avançado: nota no centro SEGUIDA por nota não-central
    dentro de um janela curta (difícil de ver a tempo).
    Diferente do simples 'notas no centro / total'.
    """
    if not all_notes:
        return {"advanced_vision_block_count": 0, "vision_block_severity": 0.0}

    WINDOW_SEC = 0.3  # janela após a nota central
    vb_count = 0
    ...
    for i, note in enumerate(all_notes):
        if note.x not in (1, 2) or note.y not in (1, 2):
            continue
        # Verifica se há nota "atrás" dentro da janela
        j = i + 1
        while j < len(all_notes):
            ...
            if blocked.x not in (1, 2) or blocked.y not in (1, 2):
                # Nota não-central bloqueada por nota central
                vb_count += 1
                severity_scores.append(1.0 - (dt / WINDOW_SEC))  # mais próximo = mais severo
            j += 1

    return {
        "advanced_vision_block_count": vb_count,
        "vision_block_severity": sum(severity_scores) / len(severity_scores) if severity_scores else 0.0,
    }
...
def analyze_patterns(
    ...
    Recebe objetos já parseados e retorna um dicionário completo de padrões.
    """
    if not notes:
        return {}
    ...
    # Média entre mãos para as métricas de stream
    stream_metrics = {
        "stream_count":       (left_streams["stream_count"] + right_streams["stream_count"]) / 2,
        "avg_stream_length":  (left_streams["avg_stream_length"] + right_streams["avg_stream_length"]) / 2,
        "max_stream_length":  max(left_streams["max_stream_length"], right_streams["max_stream_length"]),
        "stream_note_ratio":  (left_streams["stream_note_ratio"] + right_streams["stream_note_ratio"]) / 2,
    ...
    parity_metrics = {
        ...
        "parity_break_ratio": (l_parity["parity_break_ratio"] + r_parity["parity_break_ratio"]) / 2,
    ...
    arc_metrics = {
        ...
        "arc_density": (arc_count + chain_count) / duration_seconds if duration_seconds > 0 else 0.0,
    }

    # ── Aggregate pattern complexity score ──
    # Combina as métricas mais informativas em um índice de complexidade de padrão
    pattern_complexity = (
    ...
    result = {
        **{f"pat_{k}": round(v, 4) for k, v in stream_metrics.items()},
        **{f"pat_{k}": round(v, 4) for k, v in jump_metrics.items()},
        **{f"pat_{k}": round(v, 4) for k, v in crossover_metrics.items()},
        **{f"pat_{k}": round(v, 4) for k, v in double_metrics.items()},
        **{f"pat_{k}": round(v, 4) for k, v in stack_metrics.items()},
        **{f"pat_{k}": round(v, 4) for k, v in parity_metrics.items()},
        **{f"pat_{k}": round(v, 4) for k, v in lt_metrics.items()},
        **{f"pat_{k}": round(v, 4) for k, v in dominance_metrics.items()},
        **{f"pat_{k}": round(v, 4) for k, v in obstacle_metrics.items()},
        **{f"pat_{k}": round(v, 4) for k, v in bomb_metrics.items()},
        **{f"pat_{k}": round(v, 4) for k, v in vb_metrics.items()},
        **{f"pat_{k}": round(v, 4) for k, v in arc_metrics.items()},
        "pat_pattern_complexity": round(pattern_complexity, 4),
        # Métricas separadas por mão (úteis para assimetria)
        "pat_left_stream_ratio":  round(left_streams["stream_note_ratio"], 4),
        "pat_right_stream_ratio": round(right_streams["stream_note_ratio"], 4),
        "pat_left_crossover_ratio":  round(l_cross["crossover_ratio"], 4),
        "pat_right_crossover_ratio": round(r_cross["crossover_ratio"], 4),
    }
...
MAP_STYLE_THRESHOLDS = {
    "stream":     {"pat_stream_note_ratio": 0.35, "pat_stream_bpm_avg": 160},
    "tech":       {"pat_tech_ratio": 0.25, "pat_parity_break_ratio": 0.2},
    "jump":       {"pat_jump_density": 0.15, "pat_avg_jump_distance": 2.0},
    "crossover":  {"pat_crossover_ratio": 0.12},
    "speed":      {"pat_double_ratio": 0.08},
    "obstacle":   {"pat_obstacle_density": 0.5},
    "balanced":   {},   # fallback
}
